...
for t in threads:
    sample_path = Path(f"target/criterion/grpc_read_cond/{t}/new/sample.json")
    est_path = Path(f"target/criterion/grpc_read_cond/{t}/new/estimates.json")
...
if len(x) == 0:
    print(f"Error: No benchmark data found for grpc_read_cond_bench")
    print(f"Expected to find data in: target/criterion/grpc_read_cond/*/new/")
    print(f"Please run the benchmark first: make bench-read-cond")
    exit(1)
