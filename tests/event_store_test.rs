...
use uuid::Uuid;
use dcbsd::api::{AppendCondition, Query, Event, EventStoreError, QueryItem, EventStoreAPI};
...
pub fn run_event_store_test<T: EventStoreAPI>(event_store: &T) {
    ...
    let query_type2_tag_b_or_type3_tagc = Query {
        items: vec![
            QueryItem {
                types: vec!["type2".to_string()],
                tags: vec!["tagB".to_string()],
    ...
    let query_type3_tag_c_or_type2_tag_b = Query {
        items: vec![
            QueryItem {
                types: vec!["type3".to_string()],
                tags: vec!["tagC".to_string()],
    ...
    let result = event_store.append(new.clone(), Some(AppendCondition::default()));
    ...
    let result = event_store.append(
        ...
        Some(AppendCondition {
            fail_if_events_match: Query::default(),
            after: Some(1),
        }),
    );
    assert!(matches!(result, Err(EventStoreError::IntegrityError)));
    ...
    let result = event_store.append(new.clone(), Some(AppendCondition {
    ...
    let consistency_boundary = Query {
        items: vec![
            QueryItem {
