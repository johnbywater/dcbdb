...
for t in threads:
    sample_path = Path(f"target/criterion/{GROUP_NAME}/{t}/new/sample.json")
    est_path = Path(f"target/criterion/{GROUP_NAME}/{t}/new/estimates.json")
...
if len(x) == 0:
    print(f"Error: No benchmark data found for grpc_read_bench")
    print(f"Expected to find data in: target/criterion/{GROUP_NAME}/*/new/")
    print(f"Please run the benchmark first: make bench-read")
    exit(1)
...
plt.title(f'UmaDB: Read Operations')
