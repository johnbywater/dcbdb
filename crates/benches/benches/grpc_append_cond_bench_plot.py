...
for t in threads:
    sample_path = Path(f"target/criterion/grpc_append_cond_{EVENTS_PER_REQUEST}_per_request/{t}/new/sample.json")
    est_path = Path(f"target/criterion/grpc_append_cond_{EVENTS_PER_REQUEST}_per_request/{t}/new/estimates.json")
...
if len(x) == 0:
    ...
    print(f"Expected to find data in: target/criterion/grpc_append_cond_{EVENTS_PER_REQUEST}_per_request/*/new/")
    print(f"Please run the benchmark first: make bench-append-cond-{EVENTS_PER_REQUEST}")
    exit(1)
...
plt.title(f'UmaDB: Conditional Append Operations ({EVENTS_PER_REQUEST} event{"s" if EVENTS_PER_REQUEST > 1 else ""} per request)')
